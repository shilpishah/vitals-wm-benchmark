"""Invoke a function on the deployed `vitals-hunyuan` Modal app -- the
same "look up the persistent deployed app, don't `modal run`" pattern
`remote/call.py`/`remote/call_cosmos.py`/`remote/call_wan.py` already
established (see `remote/call.py`'s own docstring for why).

Usage:
    python3 remote/call_hunyuan.py download_checkpoints
    python3 remote/call_hunyuan.py image2video --input-path frame.png --prompt "..." --save-path out.mp4
"""
import argparse
import pathlib
...
def main():
    ...
    p_i2v.add_argument("--num-frames", type=int, default=121)
    ...
    if args.command == "image2video":
        ...
        imageio.mimwrite(args.save_path, frames, fps=24, quality=8)
